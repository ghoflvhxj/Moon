...
#include "Module/Physics/CharacterBody.h"
...
void MJoltPhysics::DXQuaternionToEuler(::Vec4 q, float& outPitch, float& outYaw, float& outRoll)
{
    // normalize
...
void MJoltPhysics::AddCharacterBody(std::shared_ptr<DynamicMeshComponent> InDynamicMeshComp, FBodyCapsuleData& InBodyCapsuleData)
{
    ...
    InBodyCapsuleData.Radius = 0.12f;
    ...
    std::shared_ptr<MCharacterBodyObject> NewCharacterBody = std::make_shared<MCharacterBodyObject>(BodyConstructData, InBodyCapsuleData);
    CreateBody(shape, BodyConstructData, NewCharacterBody);

    CharacterBodies.push_back(NewCharacterBody);
}
...
void MJoltPhysics::Update()
{
    ...
    // 캐릭터 바디 업데이트
    for (auto& CharacterBody : CharacterBodies)
    {
