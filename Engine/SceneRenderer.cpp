...
SceneRenderer::SceneRenderer(std::shared_ptr<MeshComponent> pDeferredRenderMesh)
{
	...
	pDeferredRenderMesh->getMaterial()->setShader(TEXT("TexVertexShader.cso"), TEXT("DeferredShader.cso"));
	pDeferredRenderMesh->setTexture(int32ToEnum<TextureType>(0), _renderTargetList[0]->getMeshComponent()->getTexture(TextureType::Diffuse));
	pDeferredRenderMesh->setTexture(int32ToEnum<TextureType>(1), _lightDiffuseTarget->getMeshComponent()->getTexture(TextureType::Diffuse));
	pDeferredRenderMesh->setTexture(int32ToEnum<TextureType>(2), _lightSpecularTarget->getMeshComponent()->getTexture(TextureType::Diffuse));
}
...
void SceneRenderer::renderRenderTargets(void) noexcept
{
	size_t renderTargetCount = _renderTargetList.size();
	for (size_t i = 0; i < renderTargetCount; ++i)
	{
		_renderTargetList[i]->Update(0.f);
		_renderTargetList[i]->Render();
	}

	_lightDiffuseTarget->Update(0.f);
	_lightDiffuseTarget->Render();

	_lightSpecularTarget->Update(0.f);
	_lightSpecularTarget->Render();
}
